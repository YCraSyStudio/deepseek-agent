import type { TranslationCatalog } from "../Types";

export const chat = {
  chat: {
    incognitoActive: "El modo incógnito está activo. Este chat se perderá al recargar la extensión.",
    applyingPermissions: "Aplicando permisos...",
    apiKeyMissing: "Falta la API key",
    askAnythingAboutYourCode: "Pregunta cualquier cosa sobre tu código...",
    configureApiKey: "Configura primero la API key en Ajustes...",
    emptyDescription: "Pregunta sobre código, genera fragmentos o recibe razonamiento en el editor.",
    send: "enviar",
    newLine: "nueva línea",
    modelSelector: "Selector de modelo",
    model: "Modelo",
    reasoning: "Razonamiento",
    activity: "Actividad",
    activities: "Actividades",
    toolCallItems: "{count} tool calls",
    off: "Desactivado",
    low: "Bajo",
    high: "Alto",
    max: "Máximo",
    removeFile: "Quitar archivo",
    large: "Grande",
    folder: "carpeta",
    readingPath: "leyendo: {path}",
    listingPath: "listando: {path}",
    fileChanged: "Archivo modificado",
    chatMessage: "Mensaje de chat",
    stopGeneration: "Detener generación",
    sendMessage: "Enviar mensaje",
    workspacePathSuggestions: "Sugerencias de rutas del workspace",
    pathSuggestionCount: "Hay {count} sugerencias de ruta disponibles.",
    noFilesOrFoldersFound: "No se encontraron archivos ni carpetas.",
    deepseekIsThinking: "DeepSeek está pensando...",
    compactingContext: "Compactando el contexto de la conversación...",
    contextAutomaticallyCompacted: "Contexto compactado automáticamente",
    recoveringConcise: "Reintentando con una respuesta concisa...",
    loadEarlierMessages: "Cargar mensajes anteriores",
    showEarlierMessages: "Mostrar {count} mensajes anteriores",
    jumpToLatest: "Saltar al último bloque de respuesta",
    streaming: "La respuesta de DeepSeek se está transmitiendo.",
    finished: "La generación de la respuesta ha terminado.",
    queueMessage: "Poner mensaje en cola",
    interruptAndGuide: "Interrumpir y guiar",
    recoveredDrafts: "Borradores recuperados tras cerrar VS Code:",
    restoreDraft: "Restaurar",
    attachContext: "Adjuntar archivos de contexto",
    attachImages: "Adjuntar imágenes",
    attach: "Adjuntar",
    removeImage: "Quitar imagen",
    enlargeImage: "Ampliar {name}",
    closeImagePreview: "Cerrar vista de imagen",
    zoomIn: "Acercar",
    zoomOut: "Alejar",
    fitImageToWindow: "Ajustar a la ventana",
    responseCancelled: "Respuesta cancelada",
    responseInterrupted: "Respuesta interrumpida",
    usage: {
      title: "Uso",
      conversation: "Uso de la conversación",
      requests: "Peticiones",
      reported: "Informadas",
      input: "Entrada",
      output: "Salida",
      reasoning: "Razonamiento",
      cacheHit: "Caché acertada",
      cacheMiss: "Caché fallida",
      total: "Total",
      cost: "Coste",
      unavailable: "No disponible",
      breakdown: "Por tipo de petición",
      byModel: "Por modelo",
      models: "{count} modelos",
      unknownModel: "Modelo desconocido",
      partial: "Hay datos de tokens para {reported} de {requests} peticiones.",
      contextWindow: "Ventana de contexto",
      contextUsed: "{used} de {total} tokens",
      contextEstimated: "estimado",
      contextUnmeasured: "Envía un mensaje para medir la ventana de contexto.",
      contextAutoCompaction: "Compactación automática en {limit}",
      contextCompactions: "{count} compactaciones",
      compactNow: "Compactar contexto",
      compacting: "Compactando el contexto…",
      compactEmpty: "Todavía no hay contexto de conversación que compactar.",
      compactedFreed: "Contexto compactado. Se liberaron unos {tokens} tokens.",
      compactFailed: "Falló la compactación del contexto: {error}",
      phases: {
        primary: "Respuesta inicial",
        toolRound: "Tool calls",
        completionReview: "Revisión de finalización",
        progressReview: "Revisión de progreso",
        securityReview: "Revisión de seguridad",
        contextSummary: "Resumen de contexto",
        fileCompaction: "Compactación de archivos",
        visionAnalysis: "Análisis visual"
      }
    }
  }
} satisfies TranslationCatalog;
